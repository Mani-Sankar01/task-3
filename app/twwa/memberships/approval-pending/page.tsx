...
const ApprovalPendingPage = () => {
  ...
  const extractChanges = (updatedData: any): ChangeDetails[] => {
    ...
    if (
      updatedData.executiveProposer &&
      hasData(updatedData.executiveProposer)
    ) {
      changes.push({
        type: "updated",
    ...
    if (updatedData.branchDetails && hasData(updatedData.branchDetails)) {
      ...
      if (
        branchData.updateBranchSchema &&
        branchData.updateBranchSchema.length > 0
      ) {
        changes.push({
          type: "updated",
      ...
      if (
        branchData.deleteBranchSchema &&
        branchData.deleteBranchSchema.length > 0
      ) {
        changes.push({
          type: "deleted",
    ...
    if (updatedData.partnerDetails && hasData(updatedData.partnerDetails)) {
      ...
      if (
        partnerData.newPartnerDetails &&
        partnerData.newPartnerDetails.length > 0
      ) {
        changes.push({
          type: "added",
          field: "New Partners",
      ...
      if (
        partnerData.updatePartnerDetails &&
        partnerData.updatePartnerDetails.length > 0
      ) {
        changes.push({
          type: "updated",
      ...
      if (
        partnerData.deletePartnerDetails &&
        partnerData.deletePartnerDetails.length > 0
      ) {
        changes.push({
          type: "deleted",
    ...
    if (
      updatedData.machineryInformations &&
      hasData(updatedData.machineryInformations)
    ) {
      const machineryData = updatedData.machineryInformations;

      if (
        machineryData.newMachineryInformations &&
        machineryData.newMachineryInformations.length > 0
      ) {
        changes.push({
          type: "added",
          field: "New Machinery",
      ...
      if (
        machineryData.updateMachineryInformations &&
        machineryData.updateMachineryInformations.length > 0
      ) {
        changes.push({
          type: "updated",
      ...
      if (
        machineryData.deleteMachineryInformations &&
        machineryData.deleteMachineryInformations.length > 0
      ) {
        changes.push({
          type: "deleted",
    ...
    if (
      updatedData.similarMembershipInquiry &&
      hasData(updatedData.similarMembershipInquiry)
    ) {
      const inquiryData = updatedData.similarMembershipInquiry;
      const changesList = [];

      if (inquiryData.org_details)
        changesList.push(`Organization details: ${inquiryData.org_details}`);
      if (inquiryData.previous_application_details)
        changesList.push(
          `Previous application: ${inquiryData.previous_application_details}`
        );
      if (inquiryData.is_member_of_similar_org)
        changesList.push(
          `Member of similar org: ${inquiryData.is_member_of_similar_org}`
        );
      if (inquiryData.has_applied_earlier)
        changesList.push(`Applied earlier: ${inquiryData.has_applied_earlier}`);
      if (inquiryData.is_valid_member)
        changesList.push(`Valid member: ${inquiryData.is_valid_member}`);
      if (inquiryData.is_executive_member)
        changesList.push(
          `Executive member: ${inquiryData.is_executive_member}`
        );
  ...
  const handleDecline = async (changeId: string) => {
    ...
    if (!declineReason.trim()) {
      setSelectedChange(
        pendingChanges.find((change) => change.id === changeId) || null
      );
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Remove the declined change from the list
        setPendingChanges((prev) =>
          prev.filter((change) => change.id !== changeId)
        );
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className="flex flex-col items-center gap-2 text-sm text-muted-foreground">
                        ...
                        <div>
                          <Badge>{change.approvalStatus}</Badge>
                        </div>
                      </div>
                    </div>
          ...
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {selectedChange?.modifiedByEditor.fullName} on{" "}
                {selectedChange &&
                  format(
                    new Date(selectedChange.modifiedAt),
                    "MMM dd, yyyy 'at' h:mm a"
                  )}
              </DialogDescription>
            </DialogHeader>
            ...
            {selectedChange && (
              <div className="space-y-4">
                {extractChanges(selectedChange.updatedData).map(
                  (change, index) => (
                    <Card key={index}>
        ...
        <Dialog open={showDeclineDialog} onOpenChange={setShowDeclineDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Please provide a reason for declining the changes for{" "}
