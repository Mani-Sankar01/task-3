...
const BillApprovalPendingPage = () => {
  ...
  const [selectedRequest, setSelectedRequest] =
    useState<PendingBillRequest | null>(null);
  ...
  const extractChanges = (updatedData: any): BillChangeDetails[] => {
    ...
    if (updatedData.fromDate && updatedData.toDate) {
      changes.push({
        ...
        description: `Billing period updated from ${format(
          new Date(updatedData.fromDate),
          "MMM dd, yyyy"
        )} to ${format(new Date(updatedData.toDate), "MMM dd, yyyy")}`,
      });
      ...
      changes.push({
        ...
        description: `From date updated to: ${format(
          new Date(updatedData.fromDate),
          "MMM dd, yyyy"
        )}`,
      });
      ...
      changes.push({
        ...
        description: `To date updated to: ${format(
          new Date(updatedData.toDate),
          "MMM dd, yyyy"
        )}`,
      });
    }
  ...
  const handleDecline = async (id: number) => {
    ...
    if (!declineNote.trim()) {
      setSelectedRequest(
        pendingRequests.find((request) => request.id === id) || null
      );
    ...
    try {
      ...
      const errorMessage =
        ...
        error.message ||
        "Failed to decline bill changes";
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <p className="text-muted-foreground text-center">
                All bill change requests have been processed. Check back later
                for new pending requests.
              </p>
            </CardContent>
          </Card>
        ) : (
          ...
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                {selectedRequest &&
                  format(
                    new Date(selectedRequest.modifiedAt),
                    "MMM dd, yyyy 'at' h:mm a"
                  )}
              </DialogDescription>
            </DialogHeader>
            ...
            {selectedRequest && (
              <div className="space-y-4">
                {extractChanges(selectedRequest.updatedData).map(
                  (change, index) => (
                    <Card key={index}>
        ...
        <Dialog open={showDeclineDialog} onOpenChange={setShowDeclineDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Please provide a note for declining the changes for{" "}
                {selectedRequest?.billingId}
              </DialogDescription>
            </DialogHeader>
